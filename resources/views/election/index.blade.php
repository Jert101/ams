                    ...
                    <div class="mt-3 flex items-center bg-indigo-500 bg-opacity-30 px-4 py-2 rounded-lg inline-block">
                        ...
                        <span>
                            ...
                            @else
                                End date not set
                            @endif
                                <span class="ml-2 bg-white text-indigo-700 px-2 py-1 rounded-full text-xs font-medium countdown-timer" data-target-date="@if($electionSetting->voting_end_date){{ $electionSetting->voting_end_date->toISOString() }}@else{{ now()->addDays(1)->toISOString() }}@endif">@if($electionSetting->voting_end_date)Ends {{ now()->diffForHumans($electionSetting->voting_end_date, ['parts' => 2]) }}@else Not scheduled @endif</span>
                            @else
                                End date not set
                            @endif
                        </span>
                ...
                @else
                    ...
                    @if($electionSetting->voting_end_date)
                        <div class="mt-3 flex items-center bg-green-500 bg-opacity-30 px-4 py-2 rounded-lg inline-block">
                            ...
                            <span>
                                <strong>Election ended:</strong> @if($electionSetting->voting_end_date)
                                {{ $electionSetting->voting_end_date->format('M d, Y') }}
                            @else
                                End date not set
                            @endif
                                    ...
                                    <div class="py-4">
                                        ...
                                        <div id="position-{{ $position->id }}" class="mt-2 hidden">
                                            ...
                                            @if($position->isUserEligible(Auth::user()))
                                                @if(ElectionCandidate::hasExistingApplication(Auth::user()->id))
                                                    <div class="bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-4 rounded-md">
                                                        <div class="flex">
                                                            ...
                                                            <div class="ml-3">
                                                                <p class="text-sm font-medium">Multiple Applications Not Allowed</p>
                                                                <p class="text-sm mt-1">You have already submitted a candidacy application for this election period. You can only apply for one position per election.</p>
                                                            </div>
                                                        </div>
                                                    </div>
                                                @else
                        ...
                        @else
                            <div class="space-y-3">
                                @foreach($userCandidacies as $candidacy)
                                    <div class="border rounded-md p-4 mb-3 {{ $candidacy->status === 'pending' ? 'bg-yellow-100 text-yellow-800' :
                                    ($candidacy->status === 'approved' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800') }}">
                                        <div class="flex justify-between items-start">
                                            <div>
                                                <h4 class="font-bold">{{ $candidacy->position->title }}</h4>
                                                <p class="text-sm mb-2">Applied on: {{ $candidacy->created_at->format('M j, Y') }}</p>
                                                
                                                @if($candidacy->status === 'rejected')
                                                    <div class="mb-2 p-2 bg-red-50 border border-red-200 rounded text-sm">
                                                        <strong>Reason for rejection:</strong> {{ $candidacy->rejection_reason }}
                                                    </div>
                                                @endif
                                            </div>
                                            
                                            <span class="px-3 py-1 rounded-full text-xs font-bold {{ $candidacy->status === 'pending' ? 'bg-yellow-200 text-yellow-800' :
                                            ($candidacy->status === 'approved' ? 'bg-green-200 text-green-800' : 'bg-red-200 text-red-800') }}">
                                                {{ ucfirst($candidacy->status) }}
                                            </span>
                                        </div>
                                        
                                        <a href="{{ route('election.candidate', $candidacy->id) }}" class="mt-2 inline-block text-indigo-600 hover:text-indigo-900">
                                            View Application
                                        </a>
                                    </div>
                                @endforeach
    ...
    @elseif($electionSetting->status === 'voting')
        ...
        <div class="grid grid-cols-1 gap-6">
            ...
            <div>
                        ...
                        <div class="flex">
                            ...
                            <div class="ml-4">
                                ...
                                <div class="text-blue-700 mb-4">
                                    <p>There are currently no candidates to vote for. This can happen for several reasons:</p>
                                    <ul class="list-disc ml-5 mt-2 space-y-1">
                                        <li>No members have applied for any positions yet</li>
                                        <li>The election was recently set up and is awaiting applications</li>
                                    </ul>
                                </div>
                ...
                @else
                    @foreach($positions as $position)
                        @if(isset($allCandidates[$position->id]) && count($allCandidates[$position->id]) > 0)
                            <div class="bg-white rounded-lg shadow-md overflow-hidden mb-6">
                                ...
                                <div class="p-4">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        @foreach($allCandidates[$position->id] as $candidate)
                                            <div class="border rounded-lg overflow-hidden {{ in_array($candidate->id, $userVotes) ? 'border-green-500 bg-green-50' : 'border-gray-200' }}">
                                                <div class="flex items-center p-4">
                                                    <div class="flex-shrink-0 mr-4">
                                                        <div class="w-16 h-16 rounded-full bg-gray-200 flex items-center justify-center overflow-hidden">
                                                            @if($candidate->user && $candidate->user->profile_photo_path)
                                                                <img src="{{ asset('storage/' . $candidate->user->profile_photo_path) }}" alt="{{ $candidate->user ? $candidate->user->name : 'Candidate' }}" class="w-full h-full object-cover">
                                                            @else
                                                    ...
                                                    <div class="flex-1">
                                                        <h4 class="font-medium text-gray-900">{{ $candidate->user ? $candidate->user->name : 'Unknown Candidate' }}</h4>
